...
const removeDuplicatesByProperty = (property, objects) => {
    ...
    return differentValues.map(val => objects.find(obj => obj[property] === val));
}

export const getAllTeamsForUser = async (userId) => {
    const coachMembershipsQuerySnapshot = await admin.firestore().collection('memberships')
        .where('membershipTypeId', '==', 'coach')
        .where('userId', '==', userId)
        .limit(10)
        .get();

    const playerMembershipsQuerySnapshot = await admin.firestore().collection('memberships')
        .where('membershipTypeId', '==', 'player')
        .where('userId', '==', userId)
        .limit(10)
        .get();

    const coachMemberships = coachMembershipsQuerySnapshot.docs.map(doc => doc.data());
    const playerMemberships = playerMembershipsQuerySnapshot.docs.map(doc => doc.data());
    ...
    const teams = (await Promise.all(
        groupDocSnapshots.map(groupDoc => getTeamForGroup({ ...groupDoc.data(), id: groupDoc.id }))
    )).filter(x => x);
