...
import { Alert, Button, CenteredContainer, Typography } from "../ui";
...
export const PleaseVerifyEmail = () => {
    ...
    const triedToSignInWithoutVerifying = (
        <>
            ...
            <h3>
                It looks like you've already got an account, but you need to verify your email
                address before you can use the site
            </h3>
            {resent && <Alert severity='success'>Verification email resent!</Alert>}
            <Button
                onClick={() => {
    ...
    return (
        <CenteredContainer maxWidth='sm'>
            <Typography align='center'>{variantMap[variant] || null}</Typography>
        </CenteredContainer>
    );
};
