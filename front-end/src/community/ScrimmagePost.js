...
export const ScrimmagePost = ({
  ...
  const handleIntrested = async () => {
    if (isCoach) {
      ...
      setMessgae(
        `Your dont have the required permission to take action on this post`
      )
    }
  }
  ...
  return (
    <>
      <Paper
        elevation={2}
        className={`${classes.scrimmagePost} ${classes.postBackground}`}
      >
          ...
          <Box display='flex' flexDirection='row' alignItems='center'>
            <Box>
              <Typography variant='h5'>
                {scrimmage.organization_name}
              </Typography>
              {!GAMES.filter(
                (game) =>
                  game.name.toLowerCase() === scrimmage.game.toLowerCase()
              )[0]?.logo && (
