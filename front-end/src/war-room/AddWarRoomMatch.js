...
export const AddWarRoomMatch = ({ handleCancel, teams, addMatch }) => {
    ...
    const handleAdd = async () => {
        if (!selectedTeam || !gameName || !opponentTeam || !matchName || !date || !maps) {
    ...
    const memoizedRosterState = useMemo(() => {
        ...
        return rosterState;
    }, [players, team.rosters, isLoading]);
    ...
    return (
        <Box className={`${classes.form} ${classes.customScroll}`}>
            <Grid container spacing={4}>
                {/* SESSION INFO */}
                <Grid item xs={12}>
                    <Typography className={classes.matchHeading}>Add Match details</Typography>
                </Grid>
